...
rem This script execute itelf
rem fisrt parameter is ACTION pack|build|restore 
rem Second parameter is WHAT the ACTION is applied on .|all|sln
rem Third parameter is solution name only given when ACTION is "sln" so the script called itself
rem (.) => this->execute->pack|build|restore->exit
rem (all) => foreach->.dotnet.cmd sln->dotnet->foreach|execute->pack|build|restore->exit 
rem (sln) => dotnet->foeach|execute->execute->pack|build|restore->exit
rem execute will go to foreach when defautl solution was not found then it will run solutions from .dotnetconfig 
