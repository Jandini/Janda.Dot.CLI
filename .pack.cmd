...
call .version
if "%DOT_GITVERSION%" equ "" echo %%DOT_GITVERSION%% was not set && goto cleanup

echo Updating templates...
for /f %%f in ('dir /b /s template.json') do move %%f %%f.org > nul && type %%f.org | jq --arg version %DOT_GITVERSION% ".classifications += [$version]" > %%f
...
echo echo .dots %DOT_GITVERSION% %%DOTS_TYPE%% >> %DOTS%
...
set PACKAGE=%BASE_NAME%.%DOT_GITVERSION%.nupkg
echo Packing %PACKAGE%...
nuget pack .nuspec -OutputDirectory %OUTPUT_DIR% -NoDefaultExcludes -Version %DOT_GITVERSION% -Properties NoWarn=NU5105
